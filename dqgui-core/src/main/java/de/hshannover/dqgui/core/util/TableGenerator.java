...
public class TableGenerator {
    public static final String CREATE_PARTITION = "\0__PARTITION__\0";
    ...
    private void createRowLine(StringBuilder stringBuilder, int headersListSize,
        ...
        for (int i = 0; i < headersListSize; i++) {
            ...
            for (int j = 0; j < columnMaxWidthMapping.get(i) + PADDING_SIZE * 2 + 1; j++) {
    ...
    private void fillCell(StringBuilder stringBuilder, String cell, int cellIndex,
            Map<Integer, Integer> columnMaxWidthMapping) {
        if(cell.equals(TableGenerator.CREATE_PARTITION)) {
            if(cellIndex == 0)
                stringBuilder.append(TABLE_JOINT_SYMBOL);
            cell = String.join("", Collections.nCopies(columnMaxWidthMapping.get(cellIndex)+PADDING_SIZE*2+1, TABLE_H_SPLIT_SYMBOL));
